...
$start_close_results_status = mysqli_query($dbcon, $query_org);

if ($start_close_results_status-> num_rows > 0) {
...
$query_org = "SELECT `active_id`, `official_name`,`address_logo` FROM `candidate` WHERE `active_org_id`= $blong_user";
$insert = mysqli_query($dbcon, $query_org);
...
<div class="w-100 h-100 container-fluid py-5 wow fadeInUp" data-wow-delay="0.1s" style="visibility: visible; animation-delay: 0.1s; animation-name: fadeInUp;">
    <div class="container py-5">
        <div class="section-title text-center position-relative pb-3 mb-5 mx-auto" style="max-width: 600px;">
            <h1 class="mb-0">Candidates Gain Votes</h1>
        </div>
        <div class="row g-5">
            
            <?php foreach ($insert as $row) {
                $user_id = $row['active_id'];
                $query_org = "SELECT `total_get_votes` FROM `voters_vote` WHERE `user_id` ='$user_id' AND `org_id`= $blong_user";
                $insert = mysqli_query($dbcon, $query_org);
                $votes = $insert->fetch_assoc();
            ?>
                        ...
                        <div class="col-12  border-0 bg-light px-2 ">
                            ...
                            <div class="row ">
                                <div class="col-3 m-0 text-center">
                                    <img class="img-fluid w-100 h-100 " src="../<?php echo $row['address_logo']; ?>" style="width: 100px; height: 100px; object-fit: cover;" alt="">
                                </div>
                                <div class="col-5 m-0 text-center">
                                    <a href="#" class="w-100 h-100 form-control py-4"><?php echo $row['official_name']; ?>
                                    </a>
                                </div>
                                <div class="col-4 wow zoomIn" data-wow-delay="0.1s" style="visibility: visible; animation-delay: 0.1s; animation-name: zoomIn;">
                                    <div class="bg-primary shadow d-flex align-items-center justify-content-center p-4" style="height: 100px;">
                                        <div class="p-1 text-center">
                                            <h5 class="text-white mb-0">Gain Votes</h5>
                                            <h1 class="text-white mb-0" data-toggle="counter-up"><?php echo $votes['total_get_votes']; ?></h1>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<?php } else if ($start_close_results_status -> num_rows <=0)  { ?>
